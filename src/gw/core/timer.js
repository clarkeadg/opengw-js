...
const MAX_FRAME_INTERVAL = 32; /* (1/16) * 1000 ~  62 fps min */
...
export const timer_update = (step) => {
  ...
  let delta_time = 0; /* both in milliseconds */

  /* time control */
  for(let delta_time = 0; delta_time < MIN_FRAME_INTERVAL; ) {
      const current_time = timer_get_ticks();
